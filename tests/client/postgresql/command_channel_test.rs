//! The dashboard's `[ send ]` path on a PostgreSQL client: `AppState::send_to_client` injects
//! an action from outside the client's own tasks and the query reaches a NetGet PostgreSQL
...
//! is independent of it by design, which is half of what this test pins).
//!
//! Outcome semantics worth knowing: `tokio_postgres` owns the socket, so NetGet can never
//! report a truthful `bytes_sent` for a query. The honest outcome is therefore
//! `Executed { detail }` naming the row count, never `Sent`.
//!
//! Run with:
//!   ./cargo-isolated.sh test --no-default-features --features postgresql --test client -- postgresql::command_channel --test-threads=100
...
async fn wait_for_client_handle(state: &AppState, id: ClientId) {
    ...
    panic!(
        "PostgreSQL client #{} never registered a command handle",
        id.as_u32()
    );
}
...
async fn injected_query_reaches_our_own_server() {
    ...
    .await
    .expect("create postgresql client");
    ...
    let outcome = state
        .send_to_client(
            client_id,
            serde_json::json!({"type": "not_a_postgresql_action"}),
