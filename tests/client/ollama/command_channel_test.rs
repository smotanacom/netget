//! The dashboard's `[ send ]` path on an Ollama client: `AppState::send_to_client` injects a
//! `send_generate_request` from outside the client's own loop and the request reaches a stub
//! endpoint on loopback.
//!
//! Zero LLM calls and no external endpoint: a `*` static handler with no actions answers every
//! client event, the client's LLM points at an unreachable URL anyway, and the "Ollama server"
//! is a local `TcpListener` speaking just enough HTTP/1.1.
//!
//! Run with:
//!   ./cargo-isolated.sh test --no-default-features --features ollama --test client -- ollama::command_channel --test-threads=100
...
fn ollama_stub_body(request: &str) -> String {
    if request.contains("/api/generate") {
        serde_json::json!({"model": "llama2", "response": "pong", "done": true}).to_string()
    } else if request.contains("/api/tags") {
        serde_json::json!({"models": [{"name": "llama2"}]}).to_string()
    } else {
...
#[tokio::test]
async fn injected_generate_request_reaches_the_endpoint() {
    let (port, seen) = spawn_http_stub(ollama_stub_body).await;
    ...
    // The command loop awaits the API call, so the outcome describes a request that really
    // completed - but the Ollama client owns no socket, so there is no honest byte count
    // and it reports Executed rather than Sent.
    let outcome = state
        .send_to_client(
            client_id,
            serde_json::json!({
                "type": "send_generate_request",
                "prompt": "dashboard-marker",
                "model": "llama2"
            }),
    ...
    match &outcome {
        ClientSendOutcome::Executed { detail } => assert!(
            detail.contains("send_generate_request") && detail.contains("llama2"),
    ...
    assert!(
        requests
            .iter()
            .any(|r| r.contains("/api/generate") && r.contains("dashboard-marker")),
        "stub endpoint never saw the generate request: {requests:?}"
    );
...
async fn injected_unknown_action_is_rejected_and_disconnect_drops_the_handle() {
    let (port, _seen) = spawn_http_stub(ollama_stub_body).await;
    let state = new_state().await;
    let (tx, _rx) = mpsc::unbounded_channel();
    let client_id = open_client(&state, port, tx.clone()).await;
    wait_for_client_handle(&state, client_id).await;
    ...
    let outcome = state
        .send_to_client(
            client_id,
            serde_json::json!({"type": "not_an_ollama_action"}),
