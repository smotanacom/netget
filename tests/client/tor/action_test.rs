...
//!
//! Nothing here needs a circuit, a socket or a model: `execute_action` is pure, so every
//! assertion below is about bytes.
//!
//! ```bash
//! ./cargo-isolated.sh test --no-default-features --features tor \
//!     --test client -- client::tor::action_test --test-threads=100
//! ```

#![cfg(feature = "tor")]

use netget::client::tor::actions::{inbound_event_fields, TorClientProtocol};
use netget::llm::actions::client_trait::{Client, ClientActionResult};
use netget::llm::actions::protocol_trait::Protocol;
use netget::state::app_state::AppState;

fn protocol() -> TorClientProtocol {
    TorClientProtocol::new()
}
...
#[tokio::test]
async fn send_tor_data_declares_data_and_encoding_and_not_data_hex() {
    let p = protocol();
    let state = AppState::new();
    ...
    for action in p
        ...
        .into_iter()
        .chain(p.get_sync_actions())
        .filter(|a| a.name == "send_tor_data")
    {
        let names: Vec<&str> = action.parameters.iter().map(|p| p.name.as_str()).collect();
        assert_eq!(
            names,
            vec!["data", "encoding"],
            "send_tor_data should offer exactly 'data' and 'encoding'"
        );
        assert!(
...
async fn text_that_looks_like_hex_is_sent_as_its_characters() {
    for literal in ["48656c6c6f", "deadbeef", "0123456789abcdef"] {
        ...
        assert_eq!(
            sent(serde_json::json!({"type": "send_tor_data", "data": literal})),
            literal.as_bytes(),
...
async fn the_legacy_data_hex_field_still_sends_its_bytes() {
    assert_eq!(
        sent(serde_json::json!({"type": "send_tor_data", "data_hex": "48656c6c6f"})),
        b"Hello"
    );
}
...
async fn a_bad_encoding_and_a_missing_payload_are_refused_by_name() {
    ...
    let msg = refusal(serde_json::json!({"type": "send_tor_data"}));
    assert!(
        msg.contains("'data'"),
...
async fn lifecycle_verbs_map_to_the_lifecycle_results() {
    ...
    assert!(
        format!(
            "{:#}",
            protocol()
                .execute_action(serde_json::json!({"type": "send_tor_frame"}))
                .unwrap_err()
        )
        .contains("send_tor_frame"),
...
///
/// `tor_data_received` used to carry `data_hex` and nothing else, so an HTTP response — the
/// single most likely thing to come back through a circuit — arrived as hex for the model to decode
/// in its head. The pair of fields is chosen so that copying them into `send_tor_data`
...
async fn declared_event_parameters_match_the_payload() {
    ...
    let event = protocol()
        .get_event_types()
        .into_iter()
        .find(|e| e.id == "tor_data_received")
        .expect("tor_data_received is declared");
