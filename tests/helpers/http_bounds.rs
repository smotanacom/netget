//! Shared drivers for the connection bounds of the HTTP-shaped servers.
//!
//! **Two independent drivers live here, and that is deliberate rather than untidy.** Two agents
//! swept the HTTP-family and the registry/cloud family in parallel and each wrote a helper of
//! this name; the APIs do not collide (`HttpBounds` and `HttpBoundsCase`), and the protocols
//! that call them differ, so merging them into one shape would mean rewriting twenty passing
//! tests to no benefit. If a third family arrives, prefer extending one of these rather than
//! adding a third.
//!
//! Both drive the same three properties from the wire, against a server whose only routing rule
//! is `*` -> `manual`:
//!
//!   * a peer that connects and says nothing is closed at the first-byte bound;
//!   * a connection with a request **parked for a human** is NOT closed, which is what stops the
//!     first assertion from being satisfiable by a server that hangs up on everybody;
//!   * the connection past `MAX_CONNECTIONS` is refused, and the slot comes back.
//!
//! Each was verified by removing the bound it tests and watching every protocol fail; the
//! removal tables are in the two sections below.
