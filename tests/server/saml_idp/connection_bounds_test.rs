//! SAML IDP's connection bounds, driven from the wire.
//!
//! `src/server/saml_idp/mod.rs` declares three numbers and argues each beside itself:
...
//! `KeepAliveTimeout` of 5s and nginx's pool-tuned 75s. An assertion's `NotOnOrAfter` bounds a
//! credential, not a socket, and is not this number.) and
...
//! `tests/helpers/http_bounds.rs`; this file supplies only what is specific to SAML IDP.
//!
//! Run with:
...
mod saml_idp_connection_bounds {
    ...
    /// The numbers exactly as `src/server/saml_idp/mod.rs` declares them. Copied on purpose: if one
    /// of them moves, this file should be re-read rather than silently follow.
    fn case() -> HttpBoundsCase {
        HttpBoundsCase {
            base_stack: "saml-idp",
            label: "SAML-IDP-bounds",
            max_connections: 256,
            idle_secs: 60,
            startup_params: None,
