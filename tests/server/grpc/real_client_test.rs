...
/// TRAILERS frame. `src/server/grpc/mod.rs` instead writes `grpc-status` into the *initial*
/// HEADERS and never emits trailers at all (`src/server/grpc/CLAUDE.md`, "No trailers"). For a
/// successful call the body is non-empty, so `http_body_util::Full::is_end_stream()` is false
/// and hyper emits HEADERS (no END_STREAM) then DATA (END_STREAM) — a stream that ends with no
/// trailers, which grpc-go rejects with the message above.
///
/// The error path next door does **not** hit this, and the asymmetry is the whole explanation:
...
/// can currently report a failure to a real gRPC client and cannot report a success.
///
/// Why it is `#[ignore]`d rather than deleted or made to assert the breakage: this repository's
/// rule is "when a test fails, fix the implementation, not the test", and the fix is a change
/// to the serving path that the pass which found this was not scoped to make. Asserting the
/// broken behaviour would enshrine it and would fail the day somebody fixes it. So the test
/// states what correct behaviour is, and is parked until the implementation provides it.
///
/// **It is not evidence and nothing may cite it as evidence** — `metadata().e2e_testing` says
/// so explicitly. Un-`#[ignore]` it as the regression test the moment trailers are emitted.
#[tokio::test]
#[ignore = "KNOWN DEFECT: NetGet never emits HTTP/2 trailers, so grpcurl fails every \
            successful unary RPC with `server closed the stream without sending trailers`. \
            This test describes correct behaviour; un-ignore it when the server emits \
            trailers. See the doc comment above."]
