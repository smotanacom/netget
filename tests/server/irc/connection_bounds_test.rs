//! The connection bounds on a real, running IRC server, driven from the wire.
//!
...
//! `src/server/irc/mod.rs` and it hangs until its own assertion window expires, because
...
//!
//! IRC's first-message bound is a minute — twice what an ircd gives an unregistered client — while
//! its idle bound is half an hour, because a registered client may legitimately sit in a channel
//! all day without typing.
//!
//! No mock backend: the LLM endpoint is a dead port. These tests assert on *deadlines*, not on
//! answers. Loopback only.
//!
//! Run with:
//!   ./cargo-isolated.sh test --no-default-features --features irc --test server -- irc::connection_bounds --test-threads=100
...
/// `src/server/irc/mod.rs::FIRST_MESSAGE_READ_TIMEOUT`.
...
async fn start_parked_server(state: &AppState) -> (ServerId, u16) {
    ...
    .create(state, tx)
    .await
    .expect("create parked irc server");
...
async fn a_connection_whose_answer_is_parked_for_a_human_is_not_closed() {
    ...
    peer.write_all(b"NICK netgettest\r\n")
        .await
        .expect("write NICK");
...
/// `src/server/irc/mod.rs::MAX_CONNECTIONS`, which takes
/// `accept_bounded::DEFAULT_MAX_CONNECTIONS`.
const MAX_CONNECTIONS: usize = 256;

/// `src/server/irc/mod.rs::CONNECTION_CAP_REFUSAL`: IRC's own way of ending a link before registration.
