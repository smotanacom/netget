//! DynamoDB's connection bounds, driven from the wire.
//!
...
//! bounds and the cap that close that are declared in `src/server/dynamo/mod.rs`; this file is the
...
//!
//! The second check is the one specific to DynamoDB: an SDK's pooled connection is idle between calls.
//!
//! Run with:
//!   ./cargo-isolated.sh test --no-default-features --features dynamo --test server -- \
//!       dynamo::connection_bounds --test-threads=100
...
/// The numbers `src/server/dynamo/mod.rs` declares. Copied rather than imported, deliberately: a
