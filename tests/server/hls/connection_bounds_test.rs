//! The connection cap on a real, running HLS server, driven from the wire.
//!
//! `HEADER_READ_TIMEOUT` bounds how long *one* connected-but-silent peer holds a socket, a task
...
//! 2. The next one is refused with **HTTP 503 + `Retry-After`** — HTTP's own vocabulary for
//!    exactly this, and the same pair this server already sends when the model backend is
//!    saturated, so a player backs off and re-requests rather than treating the stream as dead.
...
//! `src/server/hls/mod.rs` with a bare `listener.accept().await` (and drop the permit from the
//! connection task). The over-cap peer is then admitted and, having sent no request, is
...
//! by a default one. HLS is client-speaks-first, so a peer that never sends its GET provokes no
//! model call. Loopback only.
//!
//! Run with:
//!   ./cargo-isolated.sh test --no-default-features --features hls --test server -- hls::connection_bounds --test-threads=100
...
/// `src/server/hls/mod.rs::MAX_CONNECTIONS`. Deliberately duplicated rather than imported: if
/// the constant moves, this test should be re-read rather than silently follow it.
const MAX_CONNECTIONS: usize = 256;

/// The body of `src/server/hls/mod.rs::CONNECTION_CAP_REFUSAL`.
...
async fn wait_for_port(state: &AppState, id: ServerId) -> u16 {
    ...
    panic!("HLS server #{} never bound a port", id.as_u32());
}
...
#[tokio::test]
async fn the_connection_past_the_cap_gets_a_503_with_retry_after_and_the_slot_comes_back() {
    ...
    assert!(
        ...
        "503 without Retry-After tells a player to back off for an unspecified time; this \
         server's overload path already sends the pair. Got {refusal:?}"
    );
    assert!(
        refusal.ends_with(CONNECTION_CAP_BODY),
        "the refusal's body must be the declared one, framed by the server's own response \
         builder so Content-Length cannot drift. Got {refusal:?}"
    );
    ...
    for _ in 0..100 {
        ...
        match tokio::time::timeout(Duration::from_millis(300), candidate.read(&mut buf)).await {
            // HLS is client-speaks-first: silence means this peer was admitted and is waiting
            // for its request line.
            Err(_) => {
                admitted = true;
                break;
            }
