...
//! `src/server/telnet/mod.rs` and it hangs until its own assertion window expires, because
...
//!
//! Telnet's bounds are on a human timescale — two minutes for the first line, ten for a session
//! already under way, which is Cisco IOS's `exec-timeout 10 0` for vty lines — because the other
//! end of a telnet session is usually a person at a terminal rather than a program.
//!
//! No mock backend: the LLM endpoint is a dead port. These tests assert on *deadlines*, not on
//! answers. Loopback only.
//!
//! Run with:
//!   ./cargo-isolated.sh test --no-default-features --features telnet --test server -- telnet::connection_bounds --test-threads=100
...
async fn wait_for_port(state: &AppState, id: ServerId) -> u16 {
    ...
    panic!("Telnet server #{} never bound a port", id.as_u32());
}
...
async fn start_parked_server(state: &AppState) -> (ServerId, u16) {
    ...
    .create(state, tx)
    .await
    .expect("create parked telnet server");
...
/// `src/server/telnet/mod.rs::MAX_CONNECTIONS`, which takes
/// `accept_bounded::DEFAULT_MAX_CONNECTIONS`.
const MAX_CONNECTIONS: usize = 256;

/// `src/server/telnet/mod.rs::CONNECTION_CAP_REFUSAL`: a plain notice line, which is all a byte stream with a terminal on the end can carry.
