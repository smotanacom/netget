//! Environmental Sensing Service (0x181A) startup-example bytes, checked against the SIG layout.
//!
...
//! GATT Specification Supplement (Temperature is `sint16` in 0.01 deg C; Humidity is `uint16` in 0.01 %).
//!
...
use netget::server::bluetooth_ble_environmental::actions::BluetoothBleEnvironmentalProtocol;
...
fn static_service() -> Value {
    let example = BluetoothBleEnvironmentalProtocol
...
fn the_static_example_builds_the_right_service() {
    ...
    assert_eq!(
        service["uuid"],
        Value::String(sig(0x181a)),
        "the static example must build the Environmental Sensing Service (0x181A)"
    );
    assert_eq!(
        service["primary"],
        Value::Bool(true),
        "the Environmental Sensing Service is a primary service"
    );
    ...
    let advertised = BluetoothBleEnvironmentalProtocol
        .get_startup_examples()
    ...
    assert_eq!(
        advertised,
        serde_json::json!([sig(0x181a)]),
        "the advertisement must name the service the example just built"
    );
}

/// Temperature (0x2A6E) is a `sint16` with exponent -2, so the value is hundredths of a
/// degree Celsius, little-endian. `6a08` is 0x086A = 2154 = 21.54 deg C.
///
/// Byte order is the whole point: read big-endian the same octets are 0x6A08 = 27144 =
/// 271.44 deg C, which is wrong in a way that is obvious — but `0x0102` vs `0x0201` is 2.58 vs
/// 5.13 deg C, and neither looks wrong at all.
#[test]
fn temperature_is_a_little_endian_sint16_in_hundredths_of_a_degree() {
    let bytes = initial_bytes(0x2A6E);
    assert_eq!(bytes, vec![0x6A, 0x08], "little-endian 0x086A");
    assert_eq!(bytes.len(), 2, "Temperature is exactly two octets");

    let raw = i16::from_le_bytes([bytes[0], bytes[1]]);
    assert_eq!(raw, 2154);
    assert_eq!(f64::from(raw) / 100.0, 21.54, "21.54 degrees Celsius");
}

/// Humidity (0x2A6F) is a `uint16` with exponent -2, little-endian: hundredths of a percent.
/// `c012` is 0x12C0 = 4800 = 48.00 %.
#[test]
fn humidity_is_a_little_endian_uint16_in_hundredths_of_a_percent() {
    let bytes = initial_bytes(0x2A6F);
    assert_eq!(bytes, vec![0xC0, 0x12], "little-endian 0x12C0");

    let raw = u16::from_le_bytes([bytes[0], bytes[1]]);
    assert_eq!(raw, 4800);
    let percent = f64::from(raw) / 100.0;
    assert_eq!(percent, 48.0, "48.00 % relative humidity");
    assert!(
        (0.0..=100.0).contains(&percent),
        "relative humidity is a percentage; {percent} is not one"
    );
}

/// Environmental Sensing characteristics are read-and-notify: a central can poll the current
/// value or subscribe for changes. Both are what makes this service worth having — a notify-only
/// sensor cannot be queried on connect, and a read-only one cannot report a change.
#[test]
fn both_sensors_are_readable_and_notifiable() {
    for (alias, name) in [(0x2A6Eu16, "Temperature"), (0x2A6F, "Humidity")] {
        assert!(
            has_property(alias, "read"),
            "{name} (0x{alias:04X}) must be readable"
        );
        assert!(
            has_property(alias, "notify"),
            "{name} (0x{alias:04X}) must notify, or a change is only ever seen by polling"
        );
    }
}

/// Temperature and Humidity must not carry the same bytes. They did, in effect: the static
/// example answered every read with the temperature value, so a central reading Humidity got
/// 0x086A back and decoded it as 21.54 %RH — the temperature wearing the humidity field's
/// units, and nothing about it looks wrong.
#[test]
fn temperature_and_humidity_are_not_the_same_bytes() {
    assert_ne!(
        initial_bytes(0x2A6E),
        initial_bytes(0x2A6F),
        "two sensors reading identical raw values is the tell for a copied literal"
    );
}
...
fn a_read_handler_implies_a_readable_characteristic() {
    ...
    let read_handlers = BluetoothBleEnvironmentalProtocol
        .get_startup_examples()
        .static_mode["event_handlers"]
        .as_array()
        .unwrap()
...
fn a_multi_readable_service_does_not_script_one_answer_for_every_characteristic() {
    ...
    for mode in [
        BluetoothBleEnvironmentalProtocol
            .get_startup_examples()
            .script_mode,
        BluetoothBleEnvironmentalProtocol
            .get_startup_examples()
