...
//! in mouse mode and hands everything to `BluetoothBle::spawn_with_llm_actions`. Once the radio
//! is up, the base stack owns the event loop, so every `decision=` a read, write or subscribe
...
use netget::server::bluetooth_ble_mouse::radio_start_failure;

/// The radio refusal carries its own token, names the instance, and keeps the cause.
///
/// The token matters because nothing reaches the air on this path: a BLE HID mouse that never
/// started and one whose handler declined to answer look identical to anyone who is not reading
/// the log.
#[test]
fn the_radio_refusal_is_tagged_and_names_the_instance() {
    ...
    let message = radio_start_failure("NetGet-Mouse", &err);
...
fn the_radio_refusal_is_not_blamed_on_the_model() {
    let message = radio_start_failure("NetGet-Mouse", &anyhow::anyhow!("no adapter"));
