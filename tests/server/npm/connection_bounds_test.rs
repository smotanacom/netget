//! NPM's connection bounds, driven from the wire.
//!
...
//! bounds and the cap that close that are declared in `src/server/npm/mod.rs`; this file is the
...
//!
//! The second check is the one specific to NPM: npm install fetches a packument, then a tarball, then unpacks it locally; the gap between those requests is exactly what the idle bound must tolerate.
//!
//! Run with:
//!   ./cargo-isolated.sh test --no-default-features --features npm --test server -- \
//!       npm::connection_bounds --test-threads=100
...
/// The numbers `src/server/npm/mod.rs` declares. Copied rather than imported, deliberately: a
