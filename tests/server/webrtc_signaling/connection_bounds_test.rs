//! The connection cap and the idle bound on a real, running WebRTC signaling server, driven from the
//! wire. The idle tests are described where they start, below the cap test.
//!
//! `SIGNALING_HANDSHAKE_TIMEOUT` (10s) bounds how long *one* peer holds a socket before it has
//! upgraded, and `SIGNALING_MAX_PEERS` bounds how many peers may be *registered* — but a peer
//! registers only after it has upgraded and sent a register message, so a stranger who merely
//! connects is counted by neither. The cap is set to `SIGNALING_MAX_PEERS` exactly, because one
//! connection carries at most one registered peer: a smaller number would silently replace this
//! server's declared capacity with an undocumented one and make the registry's own refusal
//! unreachable.
//!
//! Three claims:
//!
...
//!    that only opens a socket is evicted by `SIGNALING_HANDSHAKE_TIMEOUT` after ten seconds, which would
...
//!    `SIGNALING_MAX_PEERS` is a signaling `error` message, and it is unreachable here: that is
//!    a WebSocket text frame, and a peer refused at the accept has not upgraded, so it has no
...
//! `src/server/webrtc_signaling/mod.rs` with a bare `listener.accept().await` (and drop the permit from
...
//!
//! Run with:
//!   ./cargo-isolated.sh test --no-default-features --features webrtc --test server -- webrtc_signaling::connection_bounds --test-threads=100
...
/// `src/server/webrtc_signaling/mod.rs::MAX_CONNECTIONS`. Deliberately duplicated rather than imported: if
...
async fn wait_for_port(state: &AppState, id: ServerId) -> u16 {
    ...
    panic!(
        "WebRTC signaling server #{} never bound a port",
        id.as_u32()
    );
}
...
async fn start_server(state: &AppState) -> (ServerId, u16) {
    ...
    let server_id = ServerForm {
        protocol: "webrtc_signaling".to_string(),
        port: Some(0),
    ...
    .create(state, tx)
    .await
    .expect("create webrtc_signaling server");
...
const PROTOCOL: &str = "webrtc_signaling";

/// The first signal the parked test sends. Registration is answered with `registered` before the connected event, which is what parks.
const PARKED_SIGNAL: &str = "{\"type\":\"register\",\"peer_id\":\"p1\"}";

/// Whether the server legitimately writes a frame before the parked handler answers.
const PARKED_EXPECTS_FRAMES: bool = true;
...
async fn a_signal_parked_for_a_human_keeps_its_connection() {
    ...
    let intercept = state
        ...
        .await
        .into_iter()
        .find(|i| i.event_type == "webrtc_signaling_peer_connected")
        .expect("the signal is not parked as an intercept");
    state
        .resolve_intercept(intercept.id, vec![])
        .await
