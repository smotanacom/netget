//! The first-byte deadline on a real, running WebDAV server, driven from the wire.
//!
...
//!    is on the silence, not on the connection. A mounted DAV share issues request after request on one pooled connection, so a single number applied to both would break every client that mounts.
//!
//! **How this was proved to fail without the bound**: delete the `tokio::time::timeout(...)`
//! around `stream.peek(...)` in `src/server/webdav/mod.rs` and the first test hangs for
...
//!
//! Run with:
//!   ./cargo-isolated.sh test --no-default-features --features webdav,tcp --test server -- connection_bounds --test-threads=100
...
/// `src/server/webdav/mod.rs::FIRST_BYTE_READ_TIMEOUT`. Deliberately duplicated: if the
...
/// A `PROPFIND` on the root, the first thing every DAV client sends after mounting.
const FIRST_REQUEST: &[u8] = b"PROPFIND / HTTP/1.1\r\nHost: 127.0.0.1\r\nDepth: 0\r\n\
    Content-Length: 0\r\n\r\n";
...
async fn wait_for_port(state: &AppState, id: ServerId) -> u16 {
    ...
    panic!("WebDAV server #{} never bound a port", id.as_u32());
}
