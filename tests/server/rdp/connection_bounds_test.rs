//! The connection bounds on a real, running RDP server, driven from the wire.
//!
...
//!
//! **The first test fails without the bound.** RDP is client-speaks-first, so a peer that
//! connects and sends nothing leaves the server parked in `read_exact` waiting for a four-byte
//! TPKT header, holding a task, an `AppState` row and a connection slot with nothing else in the
//! process that will ever close the socket. Remove the `IdleTimeoutReader` wrapper from
//! `src/server/rdp/mod.rs` and this test hangs until its own assertion window expires.
//!
//! The wrapper rather than a `tokio::time::timeout` around one call, because the Connection
//! Request is read in two steps — the TPKT header, then the X.224 body whose length that header
//! declares — so a peer that sends `0x03` and stops would otherwise still be unbounded.
//!
//! **The second test is what stops a lazy fix.** `IdleTimeoutReader`'s deadline is armed lazily,
//! only while a read is actually pending, so the model round-trip that decides the Connection
//! Confirm — or a `manual` rule parking it for a *human*, 300 seconds by default
//! (`src/state/intercepts.rs`) — runs with no clock against it. Arm it eagerly, or wrap the
//! answer as well as the read, and the connection is closed while the person is still deciding
//! whether to admit it.
//!
...
//!
//! Run with:
//!   ./cargo-isolated.sh test --no-default-features --features rdp --test server -- rdp::connection_bounds --test-threads=100
...
/// `src/server/rdp/mod.rs::FIRST_BYTE_READ_TIMEOUT`.
...
/// A minimal TPKT-framed X.224 Connection Request: no routing cookie and no RDP_NEG_REQ, which
/// is a shape [MS-RDPBCGR] allows and this server parses.
fn connection_request() -> Vec<u8> {
    let x224 = [
        6u8,  // LI: the X.224 header length that follows, minus itself
        0xE0, // CR TPDU
        0, 0, // dstRef
        0, 0, // srcRef
        0, // class
...
async fn a_peer_that_connects_and_says_nothing_is_closed_at_the_first_bound() {
    ...
    assert!(
        read.is_ok(),
        "a peer that connected and sent no Connection Request was still holding the socket, the \
         connection task and its AppState entry after {}s — the first-byte read deadline is not \
         being applied",
        elapsed.as_secs()
    );
    ...
    assert!(
        sink.is_empty(),
        "RDP is client-speaks-first and has nothing to say to a peer that asked nothing; it \
         should close, not write. Got {:?}",
        sink
    );
    assert!(
...
async fn a_connection_whose_answer_is_parked_for_a_human_is_not_closed() {
    ...
    let mut peer = TcpStream::connect(("127.0.0.1", port))
        .await
        .expect("connect");
    peer.write_all(&connection_request())
        .await
        .expect("write the Connection Request");
    ...
    match tokio::time::timeout(Duration::from_secs(3), peer.read(&mut buf)).await {
        // Nothing to read and the socket is still open: the Connection Confirm is still being
        // decided and the peer is still being served. This is the passing case.
        Err(_) => {}
        Ok(Ok(0)) => panic!(
            "the server hung up on a connection whose Connection Confirm was parked for a human \
...
/// `src/server/rdp/mod.rs::MAX_CONNECTIONS`, which takes
/// `accept_bounded::DEFAULT_MAX_CONNECTIONS`.
const MAX_CONNECTIONS: usize = 256;

/// `src/server/rdp/mod.rs::CONNECTION_CAP_REFUSAL`: nothing — RDP's refusal is a reply to a Connection Request that a capped peer has not sent.
