...
//!
//! This profile is a thin wrapper: it prepends a sentence describing the HID Service (0x1812)
//! as a consumer control device and hands everything to
//! `BluetoothBle::spawn_with_llm_actions`. Once the radio is up, the base stack owns the event
//! loop, so every `decision=` a read, write or subscribe produces is emitted in
//! `src/server/bluetooth_ble/mod.rs` against `BluetoothBleProtocol` — including the ATT
//! Unlikely Error (0x0E) the base replies with when a handler fails. There is nothing in this
//! profile for an end-to-end test to reach on that path.
//!
...
use netget::server::bluetooth_ble_remote::radio_start_failure;

/// The radio refusal carries its own token, names the instance, and keeps the cause.
///
/// The token matters because nothing reaches the air on this path: a BLE HID remote control
...
fn the_radio_refusal_is_tagged_and_names_the_instance() {
    ...
    let message = radio_start_failure("NetGet-Remote", &err);
    ...
    assert!(
        message.contains("NetGet-Remote"),
...
fn the_radio_refusal_is_not_blamed_on_the_model() {
    let message = radio_start_failure("NetGet-Remote", &anyhow::anyhow!("no adapter"));
