//! The first-byte deadline on a real, running Kubernetes server, driven from the wire.
//!
...
//!    is on the silence, not on the connection. kubectl reuses one connection for several requests, so a single number applied to both would break a working client.
//!
//! **How this was proved to fail without the bound**: delete the `tokio::time::timeout(...)`
//! around `stream.peek(...)` in `src/server/kubernetes/mod.rs` and the first test hangs for
//! its whole 70-second window and then fails with "still holding the socket".
//!
...
//!
//! Run with:
//!   ./cargo-isolated.sh test --no-default-features --features kubernetes-server,tcp --test server -- connection_bounds --test-threads=100
...
/// `src/server/kubernetes/mod.rs::FIRST_BYTE_READ_TIMEOUT`. Deliberately duplicated: if the
...
/// `GET /version`, the first thing `kubectl` asks any apiserver.
const FIRST_REQUEST: &[u8] = b"GET /version HTTP/1.1\r\nHost: 127.0.0.1\r\n\
    User-Agent: kubectl/v1.29.4\r\nAccept: application/json\r\n\r\n";

async fn new_state() -> AppState {
    // A dead LLM endpoint. These tests assert on deadlines, not on answers, and a fail-closed
    // reply proves the connection is alive exactly as well as a successful one would.
...
async fn wait_for_port(state: &AppState, id: ServerId) -> u16 {
    ...
    panic!("Kubernetes server #{} never bound a port", id.as_u32());
}
...
async fn start_server(state: &AppState) -> u16 {
    ...
    .create(state, tx)
    .await
    .expect("create kubernetes server");
...
async fn a_peer_that_connects_and_says_nothing_is_closed_at_the_first_byte_bound() {
    ...
    assert!(
        sink.is_empty(),
        "HTTP is client-speaks-first and this server has nothing to say to a peer that asked nothing; it should close, not write. Got {:?}",
...
async fn a_peer_that_speaks_is_served_and_outlives_the_first_byte_bound() {
    // The other half of the pair: the deadline bounds silence, not the connection. A bound that
    // closed a peer 30 seconds after it connected — rather than 30 seconds after it went quiet
    // without ever speaking — would break a working client.
    let state = new_state().await;
    let port = start_server(&state).await;
    ...
    peer.write_all(FIRST_REQUEST).await.expect("write request");
