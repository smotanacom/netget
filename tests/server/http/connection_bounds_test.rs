//! HTTP's connection bounds, driven from the wire.
//!
//! `src/server/http/mod.rs` declares three numbers and argues each beside itself:
//! `FIRST_BYTE_READ_TIMEOUT` (30s — HTTP is client-speaks-first, so a peer that has sent no byte
//! has asked nothing), `IDLE_BETWEEN_REQUESTS_TIMEOUT` (75s — nginx's `keepalive_timeout`, for a
//! server whose clients are whatever the operator points at it.) and
...
//! `tests/helpers/http_bounds.rs`; this file supplies only what is specific to HTTP.
//!
//! Run with:
...
mod http_connection_bounds {
    ...
    /// The numbers exactly as `src/server/http/mod.rs` declares them. Copied on purpose: if one
