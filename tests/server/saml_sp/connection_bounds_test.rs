//! SAML SP's connection bounds, driven from the wire.
//!
//! `src/server/saml_sp/mod.rs` declares three numbers and argues each beside itself:
...
//! `tests/helpers/http_bounds.rs`; this file supplies only what is specific to SAML SP.
//!
//! Run with:
...
mod saml_sp_connection_bounds {
    ...
    /// The numbers exactly as `src/server/saml_sp/mod.rs` declares them. Copied on purpose: if one
    /// of them moves, this file should be re-read rather than silently follow.
    fn case() -> HttpBoundsCase {
        HttpBoundsCase {
            base_stack: "saml-sp",
            label: "SAML-SP-bounds",
            max_connections: 256,
            idle_secs: 60,
            startup_params: None,
