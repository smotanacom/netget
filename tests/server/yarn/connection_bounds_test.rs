//! YARN's connection bounds, driven from the wire.
//!
...
//! bounds and the cap that close that are declared in `src/server/yarn/mod.rs`; this file is the
...
//!
//! The second check is the one specific to YARN: the ResourceManager API is polled, and a poller's gap between scrapes is not a dead connection.
//!
//! Run with:
//!   ./cargo-isolated.sh test --no-default-features --features yarn --test server -- \
//!       yarn::connection_bounds --test-threads=100
...
/// The numbers `src/server/yarn/mod.rs` declares. Copied rather than imported, deliberately: a
...
fn bounds() -> HttpBounds {
    HttpBounds {
        ...
        max_connections: 256,
        probe: "GET /ws/v1/cluster/info HTTP/1.1\r\nHost: 127.0.0.1\r\n\r\n",
    }
}
