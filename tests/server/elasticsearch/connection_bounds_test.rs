//! Elasticsearch's connection bounds, driven from the wire.
//!
//! `src/server/elasticsearch/mod.rs` declares three numbers and argues each beside itself:
//! `FIRST_BYTE_READ_TIMEOUT` (30s — HTTP is client-speaks-first, so a peer that has sent no byte
//! has asked nothing), `IDLE_BETWEEN_REQUESTS_TIMEOUT` (75s — nginx's `keepalive_timeout`. A held-
//! open scroll or a `?wait_for_completion` request is work in flight, not silence, so a pooled
//! client is unaffected by this number.) and
...
//! `tests/helpers/http_bounds.rs`; this file supplies only what is specific to Elasticsearch.
//!
//! Run with:
...
mod elasticsearch_connection_bounds {
    ...
    /// The numbers exactly as `src/server/elasticsearch/mod.rs` declares them. Copied on purpose: if one
    /// of them moves, this file should be re-read rather than silently follow.
    fn case() -> HttpBoundsCase {
        HttpBoundsCase {
            base_stack: "Elasticsearch",
            label: "Elasticsearch-bounds",
            max_connections: 128,
            idle_secs: 75,
            startup_params: None,
