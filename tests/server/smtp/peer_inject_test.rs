//! The dashboard's "message this peer" / "disconnect this peer" path on an SMTP server
...
//!
//! Run with:
//!   ./cargo-isolated.sh test --no-default-features --features smtp --test server -- smtp::peer_inject --test-threads=100
...
async fn wait_for_peer_handle(state: &AppState, id: ServerId) -> u32 {
    ...
    panic!(
        "SMTP server #{} never registered a peer handle",
        id.as_u32()
    );
}

#[tokio::test]
async fn injected_smtp_action_reaches_raw_socket_and_close_sends_eof() {
    ...
    let server_id = ServerForm {
        ...
        event_handlers: Some(vec![serde_json::json!({
            ...
            "handler": {
                "type": "static",
                "actions": [ { "type": "send_smtp_greeting", "hostname": "mail.test", "message": "ready" } ]
            }
        })]),
        ..Default::default()
    }
    ...
    assert_eq!(line, "220 mail.test ready\r\n");
    ...
    let outcome = state
        .send_to_peer(
            server_id,
            conn,
            serde_json::json!({"type": "send_smtp_ok", "message": "injected"}),
