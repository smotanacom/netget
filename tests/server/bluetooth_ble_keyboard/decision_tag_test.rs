...
//! in keyboard mode and hands everything to `BluetoothBle::spawn_with_llm_actions`. Once the
//! radio is up, the base stack owns the event loop, so every `decision=` a read, write or
//! subscribe produces is emitted in `src/server/bluetooth_ble/mod.rs` against
//! `BluetoothBleProtocol` — including the ATT Unlikely Error (0x0E) the base replies with when
//! a handler fails. There is nothing in this profile for an end-to-end test to reach on that
//! path.
//!
...
use netget::server::bluetooth_ble_keyboard::radio_start_failure;

/// The radio refusal carries its own token, names the instance, and keeps the cause.
///
/// The token matters because nothing reaches the air on this path: a BLE HID keyboard that
...
fn the_radio_refusal_is_tagged_and_names_the_instance() {
    ...
    let message = radio_start_failure("NetGet-Keyboard", &err);
    ...
    assert!(
        message.contains("NetGet-Keyboard"),
...
fn the_radio_refusal_is_not_blamed_on_the_model() {
    let message = radio_start_failure("NetGet-Keyboard", &anyhow::anyhow!("no adapter"));
