//! Cycling Speed and Cadence Service (0x1816) startup-example bytes, checked against the SIG layout.
//!
...
//! Cycling Speed and Cadence Service 1.0, section 3.2 (CSC Feature is a `uint16` bitfield).
//!
...
use netget::server::bluetooth_ble_cycling::actions::BluetoothBleCyclingProtocol;
...
fn static_service() -> Value {
    let example = BluetoothBleCyclingProtocol
...
fn the_static_example_builds_the_right_service() {
    ...
    assert_eq!(
        service["uuid"],
        Value::String(sig(0x1816)),
        "the static example must build the Cycling Speed and Cadence Service (0x1816)"
    );
    assert_eq!(
        service["primary"],
        Value::Bool(true),
        "the Cycling Speed and Cadence Service is a primary service"
    );
    ...
    let advertised = BluetoothBleCyclingProtocol
        .get_startup_examples()
    ...
    assert_eq!(
        advertised,
        serde_json::json!([sig(0x1816)]),
        "the advertisement must name the service the example just built"
    );
}

/// CSC Feature (0x2A5C) is a two-octet little-endian `uint16` bitfield: bit 0 Wheel Revolution
/// Data Supported, bit 1 Crank Revolution Data Supported, bit 2 Multiple Sensor Locations.
/// `0300` is 0x0003 — wheel and crank, which is what a combined speed-and-cadence sensor is.
///
/// Big-endian here gives 0x0300, setting bits 8 and 9, which are undefined: a central reads a
/// sensor that claims neither wheel nor crank data and has nothing to ask it for.
#[test]
fn csc_feature_is_a_little_endian_uint16_bitfield() {
    let bytes = initial_bytes(0x2A5C);
    assert_eq!(bytes, vec![0x03, 0x00], "little-endian 0x0003");
    assert_eq!(bytes.len(), 2, "CSC Feature is exactly two octets");

    let flags = u16::from_le_bytes([bytes[0], bytes[1]]);
    assert_eq!(flags, 0x0003);
    assert_eq!(flags & 0b1, 1, "bit 0: Wheel Revolution Data Supported");
    assert_eq!(flags & 0b10, 0b10, "bit 1: Crank Revolution Data Supported");
    assert_eq!(flags >> 3, 0, "no bit above 2 is defined in CSC Feature");

    assert!(has_property(0x2A5C, "read"), "CSC Feature is read-only");
}

/// CSC Measurement (0x2A5B) is notify-only, and carries no invented value: its flags select
/// which of two variable-length revolution records follow, so a literal that does not match
/// its own flags octet decodes as a different field entirely.
#[test]
fn the_measurement_notifies_and_invents_no_reading() {
    assert!(
        has_property(0x2A5B, "notify"),
        "CSC Measurement (0x2A5B) is notify-only: a central subscribes, it does not read"
    );
    assert!(
        !has_property(0x2A5B, "read"),
        "CSC Measurement is not readable in the spec"
    );
    assert!(
        characteristic(0x2A5B)["initial_value"].is_null(),
        "a hand-written CSC Measurement is a fabricated ride"
    );
}
...
fn a_read_handler_implies_a_readable_characteristic() {
    ...
    let read_handlers = BluetoothBleCyclingProtocol
        .get_startup_examples()
        .static_mode["event_handlers"]
        .as_array()
        .unwrap()
