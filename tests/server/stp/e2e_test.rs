...
const CONFIG_BPDU_FRAME: [u8; 60] = [
    0x01, 0x80, 0xc2, 0x00, 0x00, 0x00, 0x00, 0x1c, 0x0e, 0x87, 0x78, 0x4f, 0x00, 0x26, 0x42,
    0x42, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x1c, 0x0e, 0x87, 0x78, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x1c, 0x0e, 0x87, 0x78, 0x00, 0x80, 0x04, 0x00,
    0x00, 0x14, 0x00, 0x02, 0x00, 0x0f, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
];

/// A Topology Change Notification BPDU frame (802.1D-2004 §9.3.2).
const TCN_BPDU_FRAME: [u8; 60] = [
    0x01, 0x80, 0xc2, 0x00, 0x00, 0x00, 0x00, 0x1c, 0x0e, 0x87, 0x78, 0x4f, 0x00, 0x07, 0x42,
    0x42, 0x03, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
async fn start_stp_over_udp(ollama_url: String, instruction: &str) -> Harness {
    ...
    state
        .set_ollama_model(Some("mock-model".to_string()))
        .await;
    state
...
async fn wait_for_status(rx: &mut UnboundedReceiver<String>, needle: &str, secs: u64) -> Option<String> {
...
async fn an_unencodable_bridge_priority_refuses_to_start() {
    let state = Arc::new(AppState::new_with_options(false, "http://127.0.0.1:1".into()));
    let server_id = state
        .add_server(ServerInstance::new(
            ServerId::new(0),
            0,
