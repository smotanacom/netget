//! Spark's connection bounds, driven from the wire.
//!
...
//! bounds and the cap that close that are declared in `src/server/spark/mod.rs`; this file is the
...
//!
//! The second check is the one specific to Spark: the monitoring API is polled on a seconds-to-minutes cycle.
//!
//! Run with:
//!   ./cargo-isolated.sh test --no-default-features --features spark --test server -- \
//!       spark::connection_bounds --test-threads=100
...
/// The numbers `src/server/spark/mod.rs` declares. Copied rather than imported, deliberately: a
...
fn bounds() -> HttpBounds {
    HttpBounds {
        ...
        max_connections: 256,
        probe: "GET /api/v1/applications HTTP/1.1\r\nHost: 127.0.0.1\r\n\r\n",
    }
}
