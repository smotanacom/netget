//! The connection bounds on a real, running NNTP server, driven from the wire.
//!
...
//! `src/server/nntp/mod.rs` and it hangs until its own assertion window expires, because
...
//!
//! NNTP is server-speaks-first, so the first test needs a greeting that does not depend on a
//! backend: a static handler supplies the `200` so the connection reaches the command loop, and
//! it is the wait for the first *command* that the bound then measures.
//!
//! No mock backend: the LLM endpoint is a dead port. These tests assert on *deadlines*, not on
//! answers. Loopback only.
//!
//! Run with:
//!   ./cargo-isolated.sh test --no-default-features --features nntp --test server -- nntp::connection_bounds --test-threads=100
...
/// 60 seconds, exempted on the grounds that "NNTP is server-speaks-first". The greeting, where there is one, is *ours*; sending it says nothing
/// about whether the peer will answer it, and NetGet's own NNTP client is precisely a peer that
/// will not - `src/client/nntp/mod.rs` reads the welcome line in its read loop and writes nothing until an action or `[ send message ]` says to, and a client created from the dashboard is routed `*` -> manual. At 60 seconds this server dropped the operator's own client while
...
async fn start_parked_server(
    ...
    .create(state, tx)
    .await
    .expect("create parked nntp server");
...
async fn a_connection_whose_answer_is_parked_for_a_human_is_not_closed() {
    ...
    // Nothing is sent: NNTP greets first, and with every event parked the greeting itself is
    // waiting on a human. A peer that has produced no bytes is nevertheless being served.
...
/// `src/server/nntp/mod.rs::MAX_CONNECTIONS`, which takes
/// `accept_bounded::DEFAULT_MAX_CONNECTIONS`.
const MAX_CONNECTIONS: usize = 256;

/// `src/server/nntp/mod.rs::CONNECTION_CAP_REFUSAL`: NNTP's service-temporarily-unavailable response, which RFC 3977 allows in place of a greeting.
...
async fn once_a_command_has_been_answered_the_idle_bound_governs_not_the_first_byte_one() {
    ...
    let n = tokio::time::timeout(Duration::from_secs(20), peer.read(&mut reply))
        ...
        .expect("read reply");
    assert!(
        reply[..n].starts_with(b"200"),
        "the static rule did not answer with 200, so what follows is not the post-answer \
...
async fn the_default_leaves_a_silent_peer_alone_for_longer_than_a_person_takes() {
    ...
    // A dashboard-created NNTP client is exactly this peer: connected, reading our greeting, and
    ...
    match tokio::time::timeout(PAST_THE_OLD_BOUND, peer.read_to_end(&mut sink)).await {
        ...
        Ok(Ok(0)) => panic!(
            "the server hung up on a silent peer within {}s. The default first-byte bound was \
             60 seconds and that is less than a person takes: NetGet's own NNTP client \
             writes nothing until someone types into [ send message ], so the operator watched \
             their own client disappear. See FIRST_COMMAND_READ_TIMEOUT in src/server/nntp/mod.rs",
