//! The connection bounds on a real, running Reverse-shell server, driven from the wire.
//!
...
//! `src/server/reverse_shell/mod.rs` and it hangs until its own assertion window expires, because
...
//!
//! The peer here is a person at `nc`, so the first-command bound is two minutes — long enough to
//! read a banner and start typing — and the idle bound is the 900 seconds hardening baselines set
//! `TMOUT` to, which is the shell's own idle convention.
//!
//! No mock backend: the LLM endpoint is a dead port. These tests assert on *deadlines*, not on
//! answers. Loopback only.
//!
//! Run with:
//!   ./cargo-isolated.sh test --no-default-features --features reverse-shell --test server -- reverse_shell::connection_bounds --test-threads=100
...
/// `src/server/reverse_shell/mod.rs::FIRST_COMMAND_READ_TIMEOUT`.
...
async fn wait_for_port(state: &AppState, id: ServerId) -> u16 {
    ...
    panic!("Reverse-shell server #{} never bound a port", id.as_u32());
}
...
async fn start_server(state: &AppState) -> u16 {
    ...
    .create(state, tx)
    .await
    .expect("create reverse_shell server");
...
async fn start_parked_server(state: &AppState) -> (ServerId, u16) {
    ...
    .create(state, tx)
    .await
    .expect("create parked reverse_shell server");
...
async fn a_connection_whose_answer_is_parked_for_a_human_is_not_closed() {
    ...
    // Nothing is sent: the session-opened event is what parks, so the operator is looking at a
    // connection whose banner a human is still writing.
...
/// `src/server/reverse_shell/mod.rs::MAX_CONNECTIONS`, which takes
/// `accept_bounded::DEFAULT_MAX_CONNECTIONS`.
const MAX_CONNECTIONS: usize = 256;

/// `src/server/reverse_shell/mod.rs::CONNECTION_CAP_REFUSAL`: a plain notice line, which is all a raw stream with a person on the end can carry.
