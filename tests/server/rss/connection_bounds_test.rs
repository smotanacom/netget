//! RSS's connection bounds, driven from the wire.
//!
...
//! bounds and the cap that close that are declared in `src/server/rss/mod.rs`; this file is the
...
//!
//! The second check is the one specific to RSS: RSS has no session at all, which is why its idle bound is the shortest in this family.
//!
//! Run with:
//!   ./cargo-isolated.sh test --no-default-features --features rss --test server -- \
//!       rss::connection_bounds --test-threads=100
...
/// The numbers `src/server/rss/mod.rs` declares. Copied rather than imported, deliberately: a
