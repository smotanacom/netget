...
//!
//! Every test drives the dashboard (`src/tui/`), which paints whole frames into the
//! alternate screen, through one `PtyScreen` — a vt100 parser fed for the life of the
//! test — and waits on the text its last key must produce. Nothing here sleeps for a
//! fixed time.
//!
...
fn spawn_netget_with_args(args: &[&str]) -> (pty_process::blocking::Pty, NetGetChild) {
    ...
    // NetGet "handles this gracefully by defaulting to 80x24 in the since-removed rolling TUI" — that is the
