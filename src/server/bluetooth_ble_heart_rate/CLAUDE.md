...
**Maturity: Experimental.** BLE Heart Rate Service (0x180D) - heart rate measurement in BPM
...
- **`0000180d-0000-1000-8000-00805f9b34fb`**
  - `00002a37-0000-1000-8000-00805f9b34fb` [notify] — initial value `0048`
  - `00002a38-0000-1000-8000-00805f9b34fb` [read] — initial value `01`
...
`tests/server/bluetooth_ble_heart_rate/gatt_examples_test.rs` pins both rules.
...
- `device_name` (string, optional) — advertised name, default `NetGet-HeartRate`
...
`tests/server/bluetooth_ble_heart_rate/` exists, is declared in `tests/server/mod.rs`, and its
...
"BLUETOOTH_BLE_HEART_RATE"` reaches this protocol's `spawn`, that the base brings the radio up, and that a
`bluetooth_ble_started` event is raised and answered by the mocked model. That is real coverage
of the *wiring* and **no coverage of the profile**: nothing in it builds the Heart Rate Service (0x180D), puts a byte
