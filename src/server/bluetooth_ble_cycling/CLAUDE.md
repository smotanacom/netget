...
**Maturity: Experimental.** BLE Cycling Speed and Cadence Service (0x1816) - wheel/crank revolution data
...
- **`00001816-0000-1000-8000-00805f9b34fb`**
  ...
  - `00002a5c-0000-1000-8000-00805f9b34fb` [read] — initial value `0300`
...
`tests/server/bluetooth_ble_cycling/gatt_examples_test.rs` pins both rules.
...
- `device_name` (string, optional) — advertised name, default `NetGet-Cycling`
...
`tests/server/bluetooth_ble_cycling/` exists, is declared in `tests/server/mod.rs`, and its
...
"BLUETOOTH_BLE_CYCLING"` reaches this protocol's `spawn`, that the base brings the radio up, and that a
`bluetooth_ble_started` event is raised and answered by the mocked model. That is real coverage
of the *wiring* and **no coverage of the profile**: nothing in it builds the Cycling Speed and Cadence Service (0x1816), puts a byte
