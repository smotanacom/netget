...
**NetGet's own memcached client (`src/client/memcached/`) is a peer these bounds can close.**
It sends nothing until the model or the operator asks for something — a dashboard-created
client answers its connect event with nothing — so, pointed at this server and left alone past
30s (or 120s after its last command), it is closed here and reports `Disconnected`. That is the
outcome a real memcached with `-o idle_timeout` produces too, and memcached has no session to
lose: the client reports the close rather than keeping the socket alive.
