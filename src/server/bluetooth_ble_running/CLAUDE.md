# BLE Running Speed and Cadence

**Maturity: Experimental.** BLE Running Speed and Cadence Service (0x1814) - pace, cadence and stride length
...
- **`00001814-0000-1000-8000-00805f9b34fb`**
  ...
  - `00002a54-0000-1000-8000-00805f9b34fb` [read] — initial value `0300`
...
`tests/server/bluetooth_ble_running/gatt_examples_test.rs` pins both rules.
...
- `device_name` (string, optional) — advertised name, default `NetGet-Running`
...
`tests/server/bluetooth_ble_running/` exists, is declared in `tests/server/mod.rs`, and its
...
"BLUETOOTH_BLE_RUNNING"` reaches this protocol's `spawn`, that the base brings the radio up, and that a
`bluetooth_ble_started` event is raised and answered by the mocked model. That is real coverage
of the *wiring* and **no coverage of the profile**: nothing in it builds the Running Speed and Cadence Service (0x1814), puts a byte
