...
**Maturity: Experimental.** BLE HID gamepad (0x1812) - HID-over-GATT game controller
...
- **`00001812-0000-1000-8000-00805f9b34fb`**
  ...
  - `00002a4b-0000-1000-8000-00805f9b34fb` [read] — initial value `05010905a10105091901291015002501750195108102c0`
    (the Report Map; `hex::encode(HID_GAMEPAD_REPORT_DESCRIPTOR)`, not a second copy)
...
`HID_GAMEPAD_REPORT_DESCRIPTOR` and sizes the report from
`HID_GAMEPAD_INPUT_REPORT_LEN`, so the example a model copies cannot drift from the
...
- `device_name` (string, optional) — advertised name, default `NetGet-Gamepad`
...
`tests/server/bluetooth_ble_gamepad/` exists and is declared in `tests/server/mod.rs`. It holds
two things:
...
- **`report_descriptor_test.rs`** — pure unit tests over the HID report descriptor and the GATT
  ...
  item of `HID_GAMEPAD_REPORT_DESCRIPTOR` against the USB HID 1.11 item encoding, asserts
  the descriptor is byte-aligned and describes exactly `HID_GAMEPAD_INPUT_REPORT_LEN` bytes,
