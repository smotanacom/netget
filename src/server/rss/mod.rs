...
/// feed reader sends it inside its dial path, so a peer that has connected and sent no byte
...
///
/// Two minutes, and shorter than the registries in this family on purpose: RSS has no session.
/// Fetch and parse *is* the protocol, so a reader issues one request, reads the document and is
/// done - there is no second request this connection is waiting for. What the bound covers is a
/// poller that left a keep-alive connection open, and pollers run on a minutes-to-hours cycle
/// and reconnect when they come back. Two minutes is still above nginx's `keepalive_timeout`
/// default of 75s, so no conformant reader can be surprised by it.
const IDLE_BETWEEN_REQUESTS_TIMEOUT: std::time::Duration = std::time::Duration::from_secs(120);

/// Concurrent connections this server admits.
///
/// Each admitted connection may hold one whole in-memory feed document, so the cap is what turns
/// that per-connection bound into a total one. A feed is read by a handful of pollers, so 256 is
/// far above any real reader population and far below what socket exhaustion needs.
const MAX_CONNECTIONS: usize = crate::server::accept_bounded::DEFAULT_MAX_CONNECTIONS;

/// What a peer over [`MAX_CONNECTIONS`] is told before the socket closes.
///
/// `503 Service Unavailable` with `Retry-After`, in RSS's own vocabulary:
/// every feed reader is an HTTP client and reads a 503 as an
/// invitation to come back later, and a human with `curl` reads it directly.
                    ...
                    Ok((stream, peer_addr, permit)) => {
                        ...
                        task_owner
                            .spawn_server_task(server_id, async move {
                                // Cloned before the first-byte bound below, because the close
                                // path needs it whether or not this peer ever spoke.
                                ...
                                if spoke {
                                    ...
                                    tokio::select! {
                                        ...
                                        _ = watch_idle(
                                            ...
                                            debug!(
                                                "RSS connection {} idle for {}s; closing",
                                                connection_id,
                                    ...
                                    debug!(
                                        "RSS peer {} sent no request within {}s; closing \
