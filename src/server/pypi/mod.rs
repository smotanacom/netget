...
/// installer sends it inside its dial path, so a peer that has connected and sent no byte
...
///
/// Five minutes: twenty times pip's own socket timeout (`--timeout`, 15 seconds by default), so
/// a pip still waiting on us has long since given up before this fires, and four times nginx's
/// `keepalive_timeout` default of 75s. What is left inside the window is the gap while pip
/// resolves the next requirement or installs the wheel it just fetched - local work, on a
/// connection it may reuse afterwards. RFC 9112 requires every client to retry a request on a
/// persistent connection the server closed, and pip's urllib3 pool does exactly that.
const IDLE_BETWEEN_REQUESTS_TIMEOUT: std::time::Duration = std::time::Duration::from_secs(300);

/// Concurrent connections this server admits.
///
/// Each admitted connection may hold one whole in-memory response body - a simple-index page or
/// a wheel - so the cap is what turns that per-connection bound into a total one. pip fetches
/// essentially serially, so 256 is far above any real installer and far below what socket
/// exhaustion needs.
const MAX_CONNECTIONS: usize = crate::server::accept_bounded::DEFAULT_MAX_CONNECTIONS;

/// What a peer over [`MAX_CONNECTIONS`] is told before the socket closes.
///
/// `503 Service Unavailable` with `Retry-After`, in PyPI's own vocabulary:
/// pip treats a 503 as retryable and backs off, and a human
/// with `curl` reads it directly.
                        ...
                        task_owner
                            .spawn_server_task(server_id, async move {
                                ...
                                if spoke {
                                    ...
                                    tokio::select! {
                                        ...
                                        _ = watch_idle(
                                            ...
                                            Log::new(Some(&status_tx_clone)).debug(format!(
                                                "PyPI connection {} idle for {}s; closing",
                                                connection_id,
                                    ...
                                    Log::new(Some(&status_tx_clone)).debug(format!(
                                        "PyPI peer {} sent no request within {}s; closing \
