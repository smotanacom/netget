...
**Maturity: Experimental.** BLE Environmental Sensing Service (0x181A) - temperature, humidity and pressure
...
- **`0000181a-0000-1000-8000-00805f9b34fb`**
  - `00002a6e-0000-1000-8000-00805f9b34fb` [read, notify] — initial value `6a08`
  - `00002a6f-0000-1000-8000-00805f9b34fb` [read, notify] — initial value `c012`
...
**A `static` handler cannot tell one characteristic from another.** This service has two readable characteristics (Temperature 0x2A6E and
Humidity 0x2A6F), so its read handler is a **script** in both the script-mode and
static-mode examples, and only the GATT layout is static. A fixed
...
`tests/server/bluetooth_ble_environmental/gatt_examples_test.rs` pins both rules.
...
- `device_name` (string, optional) — advertised name, default `NetGet-Environmental`
...
`tests/server/bluetooth_ble_environmental/` exists, is declared in `tests/server/mod.rs`, and its
...
"BLUETOOTH_BLE_ENVIRONMENTAL"` reaches this protocol's `spawn`, that the base brings the radio up, and that a
`bluetooth_ble_started` event is raised and answered by the mocked model. That is real coverage
of the *wiring* and **no coverage of the profile**: nothing in it builds the Environmental Sensing Service (0x181A), puts a byte
