...
**Maturity: Experimental.** BLE Weight Scale Service (0x181D) - weight measurement and scale features
...
- **`0000181d-0000-1000-8000-00805f9b34fb`**
  - `00002a9d-0000-1000-8000-00805f9b34fb` [indicate]
  - `00002a9e-0000-1000-8000-00805f9b34fb` [read] — initial value `01000000`
...
`tests/server/bluetooth_ble_weight_scale/gatt_examples_test.rs` pins both rules.
...
- `device_name` (string, optional) — advertised name, default `NetGet-WeightScale`
...
`tests/server/bluetooth_ble_weight_scale/` exists, is declared in `tests/server/mod.rs`, and its
...
"BLUETOOTH_BLE_WEIGHT_SCALE"` reaches this protocol's `spawn`, that the base brings the radio up, and that a
`bluetooth_ble_started` event is raised and answered by the mocked model. That is real coverage
of the *wiring* and **no coverage of the profile**: nothing in it builds the Weight Scale Service (0x181D), puts a byte
