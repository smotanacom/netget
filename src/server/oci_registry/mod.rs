...
///
/// Five minutes, and the number is about the *gaps* in a pull rather than the pull. `crane pull`
/// and `docker pull` fetch a manifest, then each blob, verifying digests and writing layers to
/// disk in between; a blob being served is not silence - the response is built under a busy
/// guard and written by hyper - so what is measured here is only the pause between two requests
/// on a connection the client is keeping. Five minutes is four times nginx's `keepalive_timeout`
/// default of 75s, and a client whose idle connection is closed simply opens another, as RFC
/// 9112 requires of it.
const IDLE_BETWEEN_REQUESTS_TIMEOUT: std::time::Duration = std::time::Duration::from_secs(300);

/// Concurrent connections this server admits.
///
/// Each admitted connection may hold one whole in-memory blob or manifest body, so the cap is
/// what turns that per-connection bound into a total one. Registry clients pull layers with a
/// small fixed concurrency (containerd's default is three), so 256 is far above any real pull
/// and far below what socket exhaustion needs.
const MAX_CONNECTIONS: usize = crate::server::accept_bounded::DEFAULT_MAX_CONNECTIONS;

/// What a peer over [`MAX_CONNECTIONS`] is told before the socket closes.
///
/// `503 Service Unavailable` with `Retry-After`, in OCI registry's own vocabulary:
/// the distribution specification's own clients retry a 503 with
/// backoff, and a human with `curl` reads it directly.
                            ...
                            .spawn_server_task(server_id, async move {
                                ...
                                if spoke {
                                    ...
                                    let service = service_fn(move |req: Request<Incoming>| {
                                        ...
                                        // Cloned here rather than inside the future: the closure
                                        // is an `Fn` and an `async move` block would move the
                                        // captures out of it on the first call.
                                        let llm_client = llm_client.clone();
                                        let app_state_service = app_state_service.clone();
                                        let status_tx_service = status_tx_service.clone();
                                        let protocol = protocol.clone();
                                        async move {
                                    ...
                                    tokio::select! {
                                        ...
                                        _ = watch_idle(
                                            ...
                                            Log::new(Some(&status_tx_conn)).debug(format!(
                                                "OCI registry connection {} idle for {}s; closing",
                                                connection_id,
                                    ...
                                    Log::new(Some(&status_tx_conn)).debug(format!(
                                        "OCI registry peer {} sent no request within {}s; closing \
