...
/// DynamoDB client sends it inside its dial path, so a peer that has connected and sent no byte
...
///
/// Three minutes. DynamoDB is strictly request/response over a pooled connection and the SDKs'
/// own per-attempt read timeouts are measured in seconds, so a client still waiting on us has
/// given up long before this; what is left is a pool holding a connection it is not using. Three
/// minutes is three times the 60-second idle timeout AWS's own load balancers ship, and an SDK
/// whose pooled connection was closed opens another, as RFC 9112 requires of it.
...
/// that per-connection bound into a total one. The AWS SDKs default to 50 connections per
/// endpoint, so 256 is above even a client deliberately parallelising and far below what socket
/// exhaustion needs.
const MAX_CONNECTIONS: usize = crate::server::accept_bounded::DEFAULT_MAX_CONNECTIONS;

/// What a peer over [`MAX_CONNECTIONS`] is told before the socket closes.
///
/// `503 Service Unavailable` with `Retry-After`, in DynamoDB's own vocabulary:
                        ...
                        task_owner
                            .spawn_server_task(server_id, async move {
                                ...
                                if spoke {
                                    ...
                                    tokio::select! {
                                        ...
                                        _ = watch_idle(
                                            ...
                                            Log::new(Some(&status_tx_clone)).debug(format!(
                                                "DynamoDB connection {} idle for {}s; closing",
                                                connection_id,
                                    ...
                                    Log::new(Some(&status_tx_clone)).debug(format!(
                                        "DynamoDB peer {} sent no request within {}s; closing \
