...
**NetGet's own OAuth2 client is *lazy*, so it is never the silent peer this bound closes:**
`src/client/oauth2/mod.rs` builds a `BasicClient` purely to validate configuration and drops
it; token and authorization requests happen only from actions — `PROTOCOL_QUALITY.md`'s three-
state test.
