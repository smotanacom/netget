...
- **`00001812-0000-1000-8000-00805f9b34fb`**
  ...
  - `00002a4b-0000-1000-8000-00805f9b34fb` [read] — initial value `050c0901a101150025017501950c09cd09b509b609b709b309b409e909ea09e2093009400a2302810295048103c0`
    (the Report Map; `hex::encode(HID_REMOTE_REPORT_DESCRIPTOR)`, not a second copy)
  - `00002a4d-0000-1000-8000-00805f9b34fb` [read, notify] — initial value `0000`
    (2 bytes: exactly what the report map describes)
...
`HID_REMOTE_REPORT_DESCRIPTOR` and sizes the report from
`HID_REMOTE_INPUT_REPORT_LEN`, so the example a model copies cannot drift from the
...
`tests/server/bluetooth_ble_remote/` exists and is declared in `tests/server/mod.rs`. It holds
two things:
...
- **`report_descriptor_test.rs`** — pure unit tests over the HID report descriptor and the GATT
  ...
  item of `HID_REMOTE_REPORT_DESCRIPTOR` against the USB HID 1.11 item encoding, asserts
  the descriptor is byte-aligned and describes exactly `HID_REMOTE_INPUT_REPORT_LEN` bytes,
...
The profile itself decides exactly one outcome. Everything after the radio is up belongs to
...
|---|---|---|---|
| Shared BLE radio would not come up | this profile, `mod.rs` | nothing — no HID service published, nothing advertising | ERROR `decision=refused_adapter_unavailable` |
